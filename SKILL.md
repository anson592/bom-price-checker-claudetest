...
4. **不预设品牌**：用户没有明确指定时，禁止默认使用 ESP32/STM32/nRF52 等任意单一品牌；必须经由 Layer 0 约束矩阵 → 候选清单 → 择一流程产生
...
8. **模块类选型规则**
   ...
   - 屏幕驱动：候选 SPI 接口（ST7789/GC9A01 等）/ 并口（ILI9341/RM68xxx）
...
展示两个版本后，用 `AskUserQuestion` 让用户选择。

> **本文档不提供版本对比示例**，因为示例容易被照搬照抄，导致版本间性能差异趋同。请按 Layer 3 推理结果填入实际型号、参数与单套预估成本。
